...
class AttendanceSupabaseUser extends BaseAuthUser {
  AttendanceSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> attendanceSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = AttendanceSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
